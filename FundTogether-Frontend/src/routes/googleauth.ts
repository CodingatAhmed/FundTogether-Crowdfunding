...
app.use(session({
  ...
  cookie: {
    secure: "auto", // Set to true in production with HTTPS
    httpOnly: false // Or 'none' if cross-origin with secure: true
  }
}));
...
app.get('/routes/googleauth', async (req: any, res: any) => {
...
app.get("/routes/callbackToken", async (req, res) => {
  ...
  } else if (q.state !== (req.session as any).state) { //check state value
    console.log('State mismatch. Possible CSRF attack');
    res.end('State mismatch. Possible CSRF attack');
  }
  else {

    let takeToken = await oauth2Client.getToken(q.code as any);
  ...
  // console.log(req.query)
  app.get('/routes/getData', async (req: any, res: any) => {
    res.json(req.query);
  })
